...
files <- c("./data/fl_17.csv", "./data/sp_17.csv")
for (file in files) {
    dat <- read.csv(file)
    colnames(dat)
    ...
    #TODO: Should we reverse this metric?
    tfl_vis_scal <- dat[,grep('tfl_(1|7|13R)$', colnames(dat))]#Perseverance
    ...
    #These alphas are from the FL17 dataset.  
    alpha(uh_scal, check.keys = TRUE) # all good (.92)
    alpha(ili_scal, check.keys = TRUE) # all good (0.88) 
    alpha(ffmq_scal, check.keys = TRUE) # good (0.77)
    alpha(grt_scal, check.keys = TRUE)  # all good 0.76
    alpha(auth_scal, check.keys = TRUE) # good 0.85
    alpha(tfl_scal, check.keys = TRUE) # good 0.9
    alpha(uh.vmi_scal, check.keys = TRUE) # good 0.88 
    alpha(dis_scal, check.keys = TRUE) #A little low at .67. Remove item 3 for .68. Perhaps item 3 is inappropriate (asks about aspirin) for this cadet sample?
    alpha(leadChal_scal, check.keys = TRUE)   # (0.92)
    alpha(sc.hw_scal, check.keys = TRUE) # 0.89
